    ...
    try {
      ...
      if (isPro) {
        try {
          ...
          const newFreetextScore = (lbData?.freetextScore || 0) + evaluation.score;
          const newFreetextCount = (lbData?.freetextCount || 0) + 1;
          const mcCorrect = lbData?.mcCorrect || 0;
          const mcTotal = lbData?.mcTotal || 0;
          ...
          await leaderboardRef.set({
            ...
            createdAt: lbData?.createdAt || admin.firestore.FieldValue.serverTimestamp(),
          }, { merge: true });
        } catch (lbError) {
...
ANTWORTFORMAT (antworte NUR mit diesem JSON, kein Markdown, keine Backticks):
{
  ...
  "topic": "<eine der Themen-Kategorien aus der Liste oben>"
    ...
    try {
      ...
      const validDistractors: MCDistractor[] = Array.isArray(mcData.distractors)
        ? mcData.distractors.map((d: any) => ({
            text: typeof d === 'string' ? d : (d?.text ?? ''),
            wrongReason: typeof d === 'object' ? (d?.wrongReason ?? '') : '',
      ...
      return {
        ...
        correctReason: mcData.correctReason ?? '',
...
export const redeemProCode = functions
  .region('europe-west1')
  .https.onCall(async (data: RedeemProCodeRequest, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    if (!/^[A-Z0-9]{6}$/.test(code)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Code muss aus 6 Buchstaben und Zahlen bestehen.'
      );
    }
    ...
    if (!proCodeDoc.exists) {
      throw new functions.https.HttpsError(
        'not-found',
        'Code nicht gefunden. Prüfe die Schreibweise.'
      );
    }
    ...
    const expiresAt = proCode.expiresAt as admin.firestore.Timestamp | undefined;
    if (expiresAt && expiresAt.toDate() < new Date()) {
      throw new functions.https.HttpsError(
        'deadline-exceeded',
        'Dieser Code ist abgelaufen.'
...
export const getProCodeByOrderId = functions
  .region('europe-west1')
...
interface UpdateMCScoreRequest {
  correct: boolean;
}

export const updateMCScore = functions
  .region('europe-west1')
  .https.onCall(async (data: UpdateMCScoreRequest, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated'
    ...
    const displayName = userData.leaderboardDisplayName
      || `IT-Held ${uid.substring(0, 4).toUpperCase()}`;
    const newMcTotal = (lbData?.mcTotal || 0) + 1;
    const newMcCorrect = (lbData?.mcCorrect || 0) + (correct ? 1 : 0);
    const freetextScore = lbData?.freetextScore || 0;
    const freetextCount = lbData?.freetextCount || 0;
    const streak = userData.streak || 0;
...
function pickRandomTerm(terms: string[]): string {
  if (terms.length === 0) return FALLBACK_DAILY_TERMS[0];
  return terms[Math.floor(Math.random() * terms.length)];
}

async function loadDailyTerms(): Promise<string[]> {
  try {
    const snap = await admin.firestore().collection('dailyTerms').get();
    if (snap.empty) return FALLBACK_DAILY_TERMS;
    const terms = snap.docs
      .map((d) => (d.data().term as string | undefined)?.trim())
...
function currentBerlinTimeSlot(): { slotStart: string; slotEnd: string; dateKey: string } {
...
function isInCurrentSlot(notificationTime: string, slotStart: string, slotEnd: string): boolean {
...
export const sendDailyChallenge = functions
  .region('europe-west1')
  ...
  .onRun(async () => {
    ...
    for (const userDoc of snap.docs) {
      ...
      const lastPush = data.lastDailyPushDate as string | undefined;
...
export const testDailyChallenge = functions
  .region('europe-west1')
  .https.onCall(async (_data, context) => {
    ...
    if (!fcmToken) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Kein FCM-Token vorhanden. Aktiviere zuerst Benachrichtigungen.'
      );
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        'internal',
        `Push fehlgeschlagen: ${err?.code ?? err?.message ?? 'unbekannt'}`
      );
    }
  });
